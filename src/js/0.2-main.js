(function() {
    ...
    var mainCtrl = aB.controller('mainCtrl', ['$scope', '$log', '$http', '$localStorage', '$httpParamSerializerJQLike', '$location', '$routeParams', function($scope, $log, $http, $localStorage, $httpParamSerializerJQLike, $location, $routeParams) {
        ...
        $scope.exportPHP = function() {
            //http://localhost:8888/wordpress/?pID=102&dType=plugin&download=true
            var downloadURL = $scope.rootPath + '?pID=' + $scope.projectID + '&dType=' + this.exportType + '&download=true';
        ...
        $scope.putSource = function(menu, tab, field) {
            ...
            switch (menuType) {
                case 'cPage':
                    final = '$' + fieldName + ' = $abGen->getField(\'abOption_' + menuType + '_' + menuName + '\',\'' + tabName + '\',\'' + fieldName + '\');';
                    break;
                default:
                    final = '$' + fieldName + ' = get_post_meta(get_the_ID(),"abMB_' + tabName + fieldName + '",true);';
        ...
        //
        $scope.deleteProject = function(alertShow) {
          alertShow = (typeof alertShow !== "undefined" ? alertShow : false);

            var pID = $scope.pEditID;
            if (pID !== null) {
